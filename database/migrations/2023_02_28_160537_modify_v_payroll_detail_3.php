    ...
    public function up()
    {
        DB::statement("CREATE OR REPLACE VIEW v_payroll_detail AS 
                select 
                    ...
                    1 as num,
                    (case
                        WHEN DAYOFMONTH(d.payroll_date) BETWEEN 1 AND 7 AND WEEKDAY(d.payroll_date) = 5 THEN 1
                        WHEN DAYOFMONTH(d.payroll_date) BETWEEN 8 AND 14 AND WEEKDAY(d.payroll_date) = 5 THEN 2
                        WHEN DAYOFMONTH(d.payroll_date) BETWEEN 15 AND 21 AND WEEKDAY(d.payroll_date) = 5 THEN 3
                        WHEN DAYOFMONTH(d.payroll_date) BETWEEN 22 AND 28 AND WEEKDAY(d.payroll_date) = 5 THEN 4
                        ELSE 0 
                    end) as week_count,
                ...
                select 
                    ...
                    null as nd_hrs,
                    null as leave_balance,
                    (case when pr.basic_rate is null then count(d.time_in) * es.daily_rate else pr.basic_rate end) as debit,
                    null as credit
                from dtr d
                left join employees e on e.id = d.employee_id
                left join employee_salary es on e.id = es.employees_id 
                left join department dep on dep.id = e.department_id
                left join piece_rate pr on pr.employee_id = e.id
